...
class SCNBoxViewController: UIViewController,ARSCNViewDelegate {

    var sceneView = ARSCNView()
    
    var planes = [UUID:PlaneBox]() // 字典，存储场景中当前渲染的所有平面
    ...
    func setupRecognizers() {
        // 轻点一下就会往场景中插入新的几何体
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(SCNBoxViewController.handleTapFrom(recognizer:)))
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        ...
        let plane = PlaneBox(withAnchor: anchor, isHidden: false)
